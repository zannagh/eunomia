 ...
 * different servers never see each other's data. {@link #key} is the {@code KeyPath} (slash-joined) for keyed
 * packets and {@code null} for plain ones; {@link #replicated} marks a {@code Replicated} payload the server must
 * store and push to newcomers.
 */
public class PacketEnvelope {
    ...
    public PacketEnvelope(String scope, String channel, String key, boolean replicated, String sender, JsonElement payload) {
        this.scope = scope;
